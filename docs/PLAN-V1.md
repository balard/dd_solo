...
## Phase 1 — SAIs A: result generators

**Deliverable.** `ruleSet.sai: 'inert' | 'results' | 'full'`, and `'results'` implemented — the
SAIs that only add results, plus rerolls.

`Counter`, `Volley`, `Fly`, `Hoof`, `Trample`, `Create Fireminions`, `Smite`, `Surprise`, `Rend`,
the maneuver halves of `Firewalking` and `Teleport`, and the save half of `Rise from the Ashes`.

Four of these are not pure arithmetic and are where the phase's real work is:

- **Counter and Volley inflict damage during a save roll.** Combat currently has one damage
  direction per exchange. These add a second, and "only save results generated by spells may reduce
  this damage" means it bypasses the normal save channel entirely.
- **Smite's damage cannot be saved.** `CombatState.damage` splits into `savable` and `unsavable`.
- **Surprise suppresses the counter-attack offer** — a flag on `CombatState`, read by
  `offer_counter`.
- **Rend rerolls the die.** The first thing in the project that consumes randomness conditionally
  on a roll's own outcome. Reroll order is fixed by the rules (step 3, attackers before defenders),
  which is what keeps it deterministic.

**Exit criterion.** `sai: 'results'` plays 1000 fuzz games clean. `sai: 'inert'` still works and
still produces the v0 results — the flag is real, not decorative.

**Tests.**

| Case | Expected |
|---|---|
| Trample in a maneuver roll | contributes maneuver *and* melee; only maneuver counts here |
| Counter on a save vs melee | saves counted, and damage dealt back to the attacker |
| Counter on a save vs missile | saves only, no damage |
| Smite ×2 vs an army with 6 saves | 2 damage lands regardless of the save total |
| Rend | die rolls twice, RNG counter advances by exactly 2, replay reproduces both |
| Surprise | no `choose_counter_attack` pending is raised |
| `sai: 'inert'` | every SAI face still yields 0 and `faceResults` still throws on `'full'` |

**Bump `SAVE_VERSION`.** Rerolls change dice consumption.
...
**The fuzz gets slower and more valuable.** `npm test` is already 30–50s, most of it the 1000-game
fuzz. Dragons and spells make each game longer. `advance` throws after 1000 steps, which is a
generous bound for v0 and may not be for a game with summoning and resurrection — expect to raise
it, and be suspicious the first time you do.
